...
const searchPhoto = async (query, page) => {
	try {
		const searchHandler = await fetch(
			`${searchUrl}&page=${page}&query=${query}`
		);
...
const topicPhotos = async (topic, page) => {
	try {
		const topicHandler = await fetch(
			`https://api.unsplash.com/topics/${topic}/photos/?page=${page}&per_page=30&client_id=${api.clientId}`
		);
